...
\justify
Todos os esforços calculados pelo ProGD consideram que a carga no gancho é majorada pelo fator dinâmico onboard (Factored Load). Tal fator é calculado conforme equação prevista na API2C. Note que o valor $(\underline{P_c} + P_{moi}) \times 2,204623$ representa o SWLH do guindaste já transformado em kgf.
...
\end{equation}
O valor do $\underline{E_{sl}}$ calculado corresponde a um esforço estático. Devemos considerar a eficiência do sistema de roldanas como um valor médio, uma vez que à medida que o cabo passa pelas roldanas o esforço de tração vai aumentando. Assim, o valor efetivo do esforço de sustentação da lança deve ser $\underline{E_{sl}} \times \left[ \left( \frac{\frac{1}{E_{fl}} - 1}{2}\right) +1\right]$.
...
No cálculo das rigidezes dos cabos, os valores obtidos da interface gráfica são em $N/mm^2$. Devemos multiplicá-los por 101971621 para obter o valor em $kgf/m^2$.
\justify
[$kgf/m^2$]
\begin{equation}
...
[$kgf/m^2$]
\begin{equation}
\underline{k_{SL}} = \frac{\underline{k_{clan}} \times k_{pend}}{\underline{k_{clan}} + k_{pend}}
\end{equation}
...
Uma vez que já temos o valor do esforço de sustentação da lança ($\underline{E_{sl}}$), podemos calcular o quanto o sistema de sustentação da lança irá alongar-se ($\underline{\Delta L_{SL}}$).
\justify
[m]
\begin{equation}
\underline{d_{SL}} = \frac{\underline{E_{sl}}}{\underline{K_{SL}}}
\end{equation}
...
Uma vez que já temos o valor do esforço de compressão da lança calculado ($\underline{E_{cl}}$), podemos calcular o valor da contração da lança ($\underline{d_{CL}}$).
\justify
[m]
\begin{equation}
\underline{d_{CL}} = \frac{\underline{E_{cl}}}{k_{lan}}
\end{equation}
...
\subsubsection{Com Carga no Sistema de Sustentação da Lança}

Vamos inicialmente desprezar a contração da lança.

\begin{equation}
\underline{(L_{cl}} + \underline{d_{SL}})^2 = D_l^2 + (L-N)^2 - 2 D_l(L-N) cos(\underline{\epsilon} + \underline{\Delta \epsilon_{SL}})
\end{equation}
\justify
[m]
\begin{equation}
\underline{\Delta \epsilon_{SL}} = acos \left [ \frac{(L_{cl} + d_{SL})^2 - D_l^2 - (L-N)^2}{-2 D_l (L-N)} \right] - \underline{\epsilon}
\end{equation}
...
[m]
\begin{equation}
\underline{\Delta H_{SL}^{tip}} = (L - N)(\underline{\Delta \epsilon_{SL}}) cos(\underline{\theta})
\end{equation}

\subsubsection{Com Carga de Compressão da Lança}

Agora vamos considerar a contração da lança.

\begin{equation}
(\underline{L_{cl}} + \underline{d_{SL}})^2 = (L - N - \underline{d_{CL}})^2 + D_l^2 - 2(L - N - \underline{d_{CL}})D_l cos(\underline{\epsilon} + \underline{\Delta \epsilon_{SL}} + \underline{\Delta \epsilon_{CL}})
\end{equation}
\justify
[rad]
\begin{equation}
\underline{\Delta \epsilon_{CL}} = acos \left[ \frac{(\underline{L_{cl}} + \underline{d_{SL}})^2 - (L - N - \underline{d_{CL}})^2 - D_l^2}{2(L - N - \underline{d_{CL}})D_l} \right] - \underline{\epsilon} - \underline{\Delta \epsilon_{SL}}
\end{equation}

\begin{equation}
\underline{\Delta H_{CL}^{tip}} = (L - N) \underline{\Delta \epsilon_{CL}} cos(\underline{\theta})
\end{equation}

\subsubsection{Deslocamento Total}
